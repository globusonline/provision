...
#        csv = "org,hostname,run_list,ip," + ",".join(attr_names+deploy_data) +"\n"
    ...
    def from_configfile(cls, config):
        ...
        for domain_name in config.domains:
            ...
            if config.has_org_users_file(domain_name):
                usersfile = config.get_org_users_file(domain_name)
                ...
                for line in usersfile:
                    ...
                    if type == "G":
                        cert_type = config.get_org_user_auth(domain_name)
                    else:
                        cert_type = None
                ...
                usernum = 1
                for i in range(config.get_org_num_gridusers(domain_name)):
                ...
                for i in range(config.get_org_num_nongridusers(domain_name)):
                    username = "%s-user%i" % (domain_name, usernum)
                    user = User(login = username,
                                description = "User '%s' of Organization %s" % (username, domain_name),
                                password_hash = "TODO",
            ...
            if config.has_domain_login(domain_name):            
            ...
            if config.get_org_user_auth(domain_name) == "myproxy":
            ...
            if config.has_org_gridftp(domain_name):
