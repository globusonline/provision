...
users.each do |u|
	...
	execute "ssh-keygen" do
	  not_if do File.exists?("/export/home/#{u[:login]}/.ssh/id_rsa") end
	  user u[:login]
	  command "ssh-keygen -N \"\" -f /export/home/#{u[:login]}/.ssh/id_rsa"
	  action :run
	end
	...
	# can SSH into other nodes in the organization.
        execute "add-key" do
          user u[:login]
          command "cat /export/home/#{u[:login]}/.ssh/id_rsa.pub > /export/home/#{u[:login]}/.ssh/authorized_keys"
          action :run
        end
...
users.select{|u| u[:auth_type] == :certs}.each do |u|
	directory "/export/home/#{u[:login]}/.globus" do
	  owner u[:login]
	  group "users"
	  mode "0755"
	  action :create
	...
	cookbook_file "/export/home/#{u[:login]}/.globus/usercert.pem" do
	...
	cookbook_file "/export/home/#{u[:login]}/.globus/userkey.pem" do
